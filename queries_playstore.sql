...
SELECT app_name,id, last_updated
FROM analytics
WHERE id =1880;
...
SELECT reviews, app_name, rating 
FROM analytics
WHERE rating >= 4.8
ORDER BY reviews DESC
LIMIT 1;
...
SELECT SUM(reviews) AS ALL REVIEWS SUM
FROM analytics;
...
SELECT app_name, min_installs, min_installs/reviews AS proportion
FROM analytics
